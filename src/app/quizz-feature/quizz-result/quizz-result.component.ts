  ...
  result$ = this.quizFacade.result$.pipe(
    tap((result) => {
      if (!result) {
        ...
        this.correctAnswersCount = result.answers.filter(
          (a) => a.status
        ).length;
        const maxTime = Math.max(
          ...result.answers.map((answer) => answer.timeElapsed)
        );
        this.hardest = result.answers.find(
          (answer) => answer.timeElapsed === maxTime
        )?.id;
        const minTime = Math.min(
          ...result.answers.map((answer) => answer.timeElapsed)
        );
        this.easiest = result.answers.find(
          (answer) => answer.timeElapsed === minTime
        )?.id;
      }
    })
  );
